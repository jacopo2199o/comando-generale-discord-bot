...
const messageDelete = async (deletedMessage) =>
{
  ...
  if (audits === undefined)
  {
    return console.error("message delete: audits undefined");
  }
  const audit = audits.entries.first();
  if (audit === undefined)
  {
    return console.error("message delete: audit undefined");
  }
  const author = deletedMessage.guild.members.cache.get(deletedMessage.author?.id);
  if (author === undefined)
  {
    return console.error("message delete: author undefined");
  }
  const executor = deletedMessage.guild.members.cache.get(audit.executor?.id);
  if (executor === undefined)
  {
    return console.error("message delete: executor undefined");
  }
  if (author.id === executor.id || executor.id === deletedMessage.guild.ownerId)
  {
    return;
  }

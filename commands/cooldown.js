...
async function cooldown(
  ...
  if (
    cooldowns[interaction.guild.id][member.id] === undefined
  ) {
    interaction.client.emit(
      "activity",
      member,
      customPoints.cooldownPenalty
    );
    const endInterval = new Date().getTime() + (1000 * 60 * 60 * interval);
    const endPeriod = new Date().getTime() + (1000 * 60 * 60 * period);
    cooldowns[interaction.guild.id][member.id] = {
      interval,
      endInterval,
      endPeriod
    };
    ...
    message.setDescription(
      `${role} *${member}* received ${interval} hour/s *cooldown* penalty for a period of ${period} hour/s`
    );
    message.addFields(
      {
        name: "promotion points",
        value: `${customPoints.cooldownPenalty} ⭐`,
        inline: true
      }
    );
    message.addFields(
    ...
    await interaction.editReply(
      "this member have already a *cooldown* penalty"
    );
  }
}

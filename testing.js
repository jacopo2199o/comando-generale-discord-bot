...
const jRolePointsTable = [
	{ id: "1138015362416390204", points: 4 }, // rosso
	{ id: "1137408000462688397", points: 2 }, // arancione
...
export const myTest = async (client) => {
	...
	const membersActivityPoints = (await guild.members.fetch())
		.filter(member => member.user.id !== "1149977789496311888")
		.map(member => {
			return {
				"id": member.id,
			...
			member.roles.cache.forEach(role => {
				...
				if (rolePointTable) {
					if (rolePointTable.points > memberActivityPoints.points) {
						memberActivityPoints.points = rolePointTable.points;
	...
	const updateMemberRole = (guild, memberActivityPoints) => {
		...
		if (memberActivityPoints.points < actualRole.points) {
			if (previousRole) {
				...
				const message = `<@${memberActivityPoints.id}> downgraded to <@&${previousRole.id}> from <@&${actualRole.id}>`;
				client.channels.cache.get("1100786695613456455").send({ content: message, flags: [4096] });
			}
		} else if (memberActivityPoints.points >= nextRole.points) {
			if (nextRole) {
				memberActivityPoints.roleId = nextRole.id;
				// guildMember.roles.remove(actualRole.id);
				// guildMember.roles.add(nextRole.id);
				const message = `<@${memberActivityPoints.id}> promoted to <@&${nextRole.id}> from <@&${actualRole.id}>`;
				client.channels.cache.get("1100786695613456455").send({ content: message, flags: [4096] });
			}
		}
	};
	...
	setInterval(tickCycle, 4000, guild, membersActivityPoints); // ricorda di aggiornarlo ogni giorno * 60 * 60
};
